...
# Paths
project_folder = os.path.dirname(os.path.abspath(__file__))  # Path to oglinda_smart/
local_storage_path = os.path.join(project_folder, "closet")  # Path to oglinda_smart/closet/
closet_file_path = os.path.join(project_folder, "closet_data.json")  # Path to store closet data
detected_items_file_path = os.path.join(project_folder, "detected_items.json")  # Path to detected items file
...
# Helper function to extract dominant colors using KMeans clustering
def extract_dominant_colors(image_path, k=3):
    img = Image.open(image_path).convert('RGB')  # Open and ensure RGB format
    img_np = np.array(img)
    img_reshaped = img_np.reshape((-1, 3))  # Reshape to 2D array
    kmeans = KMeans(n_clusters=k).fit(img_reshaped)  # Perform KMeans clustering
    dominant_colors = np.round(kmeans.cluster_centers_).astype(int)  # Get dominant colors
...
def rgb_to_name(rgb, threshold=100):
    ...
    for color_name, color_rgb in mcolors.CSS4_COLORS.items():
        hex_rgb = tuple(int(c * 255) for c in mcolors.hex2color(color_rgb))  # Convert hex to RGB
...
# Function to classify items as tops or bottoms
...
# Process image and detect clothing items
...
# Update closet data with detected items
...
# Main function
def main():
    image_path = 'clothes/0a1955a3-c4c5-4141-8a72-afbcf92d0dfa.jpg'  # Replace with your image path
